...
contract PaywallTokenFactoryTest is Test {
    ...
    function test_deployNftsMultiple(address user, string memory ipfsHash, uint128 spotPrice, uint128 delta) public {
        vm.assume(spotPrice > 0);
        vm.assume(user != address(0));
        ...
        for (uint256 i = 0; i < nfts.length; i++) {
            PaywallToken nft = PaywallToken(payable(nfts[i]));
            assertEq(nft._contentOwner(), user);
            assertEq(nft._ipfsHash(), ipfsHash);
            assertEq(nft._spotPrice(), spotPrice);
            assertEq(nft._delta(), delta);
